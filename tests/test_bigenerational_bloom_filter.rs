/**
 * test_bigenerational_bloom_filter.rs
 *
 * Integration tests for BigenerationalBloomFilter.
 */
use bloom::bloom::BigenerationalBloomFilter;

#[test]
/// Test that the getter for hash_count is visible from outside the crate
fn test_get_hash_count() {
    let bf: BigenerationalBloomFilter = BigenerationalBloomFilter::new(10000, 0.01);
...
fn test_false_positive_rate() {
    let mut bf: BigenerationalBloomFilter = BigenerationalBloomFilter::new(10000, 0.01);
    for i in 0..10000 {
...
fn test_e2e() {
    let mut bf: BigenerationalBloomFilter = BigenerationalBloomFilter::new(100, 0.001);
    // Fill 'er up
    for i in 0..199 {
        bf.insert(&i.to_string());
    }
